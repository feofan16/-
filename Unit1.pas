...
ADOQuery2.Insert;
ADOQuery2.FieldByName('Наименование продукции').AsString:=Edit2.Text;
ADOQuery2.FieldByName('Цена').AsInteger:=StrToInt(Edit5.Text);
 ADOQuery2.FieldByName('КоличествоМест').AsInteger:=StrToInt(Edit4.Text);
ADOQuery2.FieldByName('НомерСклада').AsInteger:=StrToInt(Edit3.Text);
 ADOQuery2.FieldByName('Дата').AsDateTime:=DateTimePicker1.DateTime;
   end;
          Button2.Click ;
...
const
  s1: string = 'Select `Места(свободно)` FROM Склад WHERE Id_склада=';
begin
 ADOQuery3.Active := False;
  ADOQuery3.SQL.Clear;
  ADOQuery3.SQL.Text := 'Select `Места(свободно)` FROM Склад WHERE Id_склада=' + Edit3.Text;
  ADOQuery3.Active := True;
       kolvo:= StrToInt(Edit4.Text);
   //  edit6.Text:=  dbgrid3.Fields[0].AsString;
     mest:= dbgrid3.Fields[0].AsInteger; //  strtoint(edit6.Text);
       if mest<kolvo then begin showmessage('Количество превышает доступного кол-во на складе'); exit; end;
    itog:= mest - kolvo;
  ...
  //   edit8.Text:=  inttostr(itog);
     //    showmessage(inttostr(mest));
         ADOQuery3.Edit;
          ADOQuery3.FieldByName('Места(свободно)').AsInteger:= itog;
Adoquery3.Post;
...
begin
     ...
     DBGrid4.visible:=true;
        ...
        ADOQuery4.SQL.Clear;
ADOQuery4.SQL.Add('select `Код продукции`, `Наименование продукции`, `Цена`, `КоличествоМест`, `НомерСклада`, `Дата` from Продукция');
...
if Edit7.Text='0' then  begin
 try
if Application.MessageBox('Подтвердить отгрузку?',' ',MB_YESNO)=IDYES then
begin
ADOQuery4.Delete;
end;
except
on e:Exception do   showmessage('Весь товар отгружен');
 end; end;   end;


    ADOQuery3.Active := False;
  ADOQuery3.SQL.Clear;
  ADOQuery3.SQL.Text := 'Select `КоличествоМест` FROM Продукция WHERE НомерСклада=' + Edit8.Text;
  ADOQuery3.Active := True;
       kolvo:= StrToInt(Edit7.Text);
   //  edit6.Text:=  dbgrid3.Fields[0].AsString;
     mest:= dbgrid3.Fields[0].AsInteger;
    if mest<kolvo then begin showmessage('Количество отгружаемой превышает кол-во на складе'); exit; end; //  strtoint(edit6.Text);
    itog:= mest - kolvo;
   // edit6.Text:= inttostr(mest);
  //  edit7.Text:=  inttostr(kolvo);
  //   edit8.Text:=  inttostr(itog);
     //    showmessage(inttostr(mest));
         ADOQuery3.Edit;
          ADOQuery3.FieldByName('КоличествоМест').AsInteger:= itog;
Adoquery3.Post;

     showmessage('Успешно отгружено');
...
begin
 ADOQuery3.Active := False;
  ADOQuery3.SQL.Clear;
  ADOQuery3.SQL.Text := 'Select `Места(свободно)` FROM Склад WHERE Id_склада=' + Edit8.Text;
  ADOQuery3.Active := True;
       kolvo:= StrToInt(Edit7.Text);
   //  edit6.Text:=  dbgrid3.Fields[0].AsString;
     mest:= dbgrid3.Fields[0].AsInteger; //  strtoint(edit6.Text);
...
if (DBGrid1.DataSource=DataSource1) then  begin
   Label3.Caption:='Склад № : '+ dbgrid1.Fields[0].AsString;
Label1.Caption:='Количество мест: '+ dbgrid1.Fields[3].AsString;
    Label4.Caption:='Свободных мест: '+ dbgrid1.Fields[4].AsString;
    if DBGrid1.DataSource=DataSource4 then  begin
           Label3.Caption:='Склад № : '+ dbgrid1.Fields[5].AsString;
Label1.Caption:='Количество мест: '+ dbgrid1.Fields[1].AsString;
...
begin
 Label4.Caption:= dbgrid4.Fields[1].AsString;
           Label3.Caption:='Склад № : '+ dbgrid4.Fields[4].AsString;
Label1.Caption:='Занимаемых мест: '+ dbgrid4.Fields[3].AsString;
...
Edit6.Text:=ADOQuery4.FieldByName('Наименование продукции').AsString;
Edit9.Text:=IntToStr(ADOQuery4.FieldByName('Цена').AsInteger);
Edit7.Text:=ADOQuery4.FieldByName('КоличествоМест').AsString;
Edit8.Text:=IntToStr(ADOQuery4.FieldByName('НомерСклада').AsInteger);
DateTimePicker2.DateTime:= ADOQuery4.FieldByName('Дата').AsDateTime;
...
ADOQuery1.Active :=false;
ADOQuery1.SQL.Clear;
ADOQuery1.SQL.Add('SELECT * FROM Склад WHERE Id_Склада LIKE' +QuotedStr('%'+Edit10.Text+'%')+ 'or Склад LIKE' +QuotedStr('%'+Edit10.Text+'%')+'or Назначение LIKE' +QuotedStr('%'+Edit10.Text+'%')+'or `Места(всего)` LIKE' +QuotedStr('%'+Edit10.Text+'%')+'or `Места(свободно)` LIKE' +QuotedStr('%'+Edit10.Text+'%'));
...
begin
  ADOQuery4.Active :=false;
ADOQuery4.SQL.Clear;
ADOQuery4.SQL.Add('SELECT `Код продукции`, `Наименование продукции`, `Цена`, `КоличествоМест`, `НомерСклада`, `Дата` FROM Продукция WHERE `Код продукции` LIKE' +QuotedStr('%'+Edit11.Text+'%')+ 'or `Наименование продукции` LIKE' +QuotedStr('%'+Edit11.Text+'%')+'or `Цена` LIKE' +QuotedStr('%'+Edit11.Text+'%')+'or `КоличествоМест` LIKE' +QuotedStr('%'+Edit11.Text+'%')+'or `НомерСклада` LIKE' +QuotedStr('%'+Edit11.Text+'%')+'or `Дата` LIKE' +QuotedStr('%'+Edit11.Text+'%'));
...
i:=10;
  ...
  edit1.Text:= inttostr(strtoint(edit1.Text)+i); end;
   Label1.Caption:= 'Количество мест: '+ edit1.Text;
    ADOQuery1.Edit;
ADOQuery1.FieldByName('Места').AsString:=Edit1.Text;
...
edit1.Visible:=false;
      if Application.MessageBox('Изменить?',' ',MB_YESNO)=IDYES then  begin
     Label1.Caption:= 'Мест всего: '+ edit1.Text;
    ADOQuery1.Edit;
ADOQuery1.FieldByName('Места').AsString:=Edit1.Text;
Adoquery1.Post;  edit1.Visible:=false;  end else exit;
end;    end;
